...
class connectDB:
    ...
    RANK_SPREADSHEET_KEY = "1_mMdApekGitmdRGSYjSUz9DUY0rNRmYB6gC6wwFo11A"
    ...
    def get_closest_airport(self, college_name):
        ...
        for row in self.worksheet_data[0][1:]:
            airport = row[0].lower()

            for nickname in row[3:]:
                if college_name in nickname.lower():
                    # Calculate the distance between the college and the airport
                    distance = 100
        ...
        if closest_airport:
            return closest_airport.upper()
        else:
            raise ValueError("No closest airport found for the given college name.")
    ...
    def analysis(self, rank, ciwg):
        ...
        for row in current_sheet:
            ...
            if difference < 0: pass
                ...
                lowclg.append(f"{row[1]} {row[2]} Closing rank: {row[5]}")
            elif difference < 100000:
                midclg.append(f"{row[1]} {row[2]} Closing rank: {row[5]}")
            if difference > 100000:
                highclg.append(f"{row[1]} {row[2]} Closing rank: {row[5]}")
    ...
    def testing(self):
        ...
        if userinput == "1":
            ...
            try:
                closest_airport = connectDB.get_closest_airport(self, college_name)
                print(f"The closest airport to {college_name} is {closest_airport}")
